...
| `flatness_map_fixed_q.pdf`, `flatness_map_fixed_w.pdf` | tile maps: rows of *W* (or *Q*), one tile per beam at its lab-momentum range, coloured by ⟨n_SD⟩; every row one colour |
| `flatness_map_inclusive.pdf` | the control: one strip per beam with no (*W*, *Q*) selection; colour drifts along every strip |
